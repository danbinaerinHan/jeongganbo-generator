...
%   왼쪽 위 Menu → Settings → Compiler 를 "XeLaTeX"으로 바꾼 뒤
%   Recompile. (Overleaf 기본값 pdfLaTeX으로는 한글 폰트를 못 쓴다.
%   latexmkrc로는 컴파일러를 못 바꾸므로 반드시 메뉴에서 설정할 것.)
%   로컬: xelatex main && bibtex main && xelatex main && xelatex main
%
...
% (XeLaTeX·LuaLaTeX은 통과)
\usepackage{iftex}
\ifPDFTeX
  \PackageError{jgb-paper}
    {Wrong compiler: this paper needs XeLaTeX}
    {In Overleaf: click Menu (top-left) -> Settings -> Compiler ->
     choose "XeLaTeX" -> Recompile. Locally: use xelatex, not pdflatex.}
\fi
...
% 한자 폰트를 따로 지정하지 않으면 율명 한자(潢 등)가 빠질 수
% 있어 한글과 같은 폰트로 함께 지정한다.
% ------------------------------------------------------------
...
\IfFontExistsTF{Noto Sans CJK KR}{%
  ...
  % 은바탕은 TeX Live 동봉 파일명으로 지정 (fontconfig 불필요)
