...
% JDCS: 표 캡션은 표 위, 국문(표 n.)+영문(Table n.) 병기, 표 내부는 영문
\begin{table}[t]
\centering\small
\caption{선율 텍스트 문법 요약\\
Table~\thetable. Summary of the melody text grammar}
\label{tab:grammar}
\begin{tabularx}{\columnwidth}{@{}lX@{}}
\toprule
Notation & Meaning \\
\midrule
Line break & Gak (vertical column) boundary \\
\code{|} & Jeonggan (one-beat cell) boundary \\
Space inside a cell & Bunbak --- vertical subdivision of the cell \\
Juxtaposition (no space) & Buchim --- horizontal grouping within a row \\
황·대·태$\cdots$응* & Twelve yulmyeong pitch names (Hangul) \\
청·중청·배·하배* & Octave prefixes ($+1,+2,-1,-2$) \\
\code{-} & Sustain of the previous note \\
\code{쉼}* & Rest (silence) \\
\code{<} after a note & Breath mark (lower-right corner of the cell) \\
\code{\{name\}} & Attach a sigimsae (e.g., \code{황\{미는표\}}) \\
\code{\{name@$s$,$x$,$y$\}} & Sigimsae scale $s$ \% and offset $x$, $y$ \\
\bottomrule
\end{tabularx}

\smallskip
\raggedright\scriptsize *Korean tokens are unavoidable here: they are the
literal input vocabulary of the notation itself.
\end{table}
...
그림~\ref{fig:mapping}에 이 문법과 조판 결과의 실제 대응을 보였다.
...
시김새는 이름 기반으로 \code{\{미는표\}}처럼 부가하며, 그래픽 사용자
인터페이스(GUI; graphical user interface)에서 개별 기호의
