...
\begin{tikzpicture}[
  ...
  \node[box] (src) {텍스트 소스\\(선율·장단·가사)};
  \node[box, right=of src] (parser) {파서};
  \node[box, right=of parser] (model) {문서 모델};
  \node[box, below=of model] (layout) {SVG 레이아웃\\엔진 (mm 단위)};
  \node[box, left=of layout] (out) {화면 / 인쇄·PDF\\/ 300dpi PNG};
  \node[box, left=of out] (audio) {Web Audio\\재생};
  ...
  \draw[arr, dashed] (out.north) to[out=120,in=-60]
    node[midway, left, font=\scriptsize]{WYSIWYG 편집} (src.south east);
\end{tikzpicture}
...
\begin{figure}[t]
\centering
\fbox{\includegraphics[width=0.86\columnwidth]{figures/fig-page.png}}
...
구현했다. 한글 입력기(IME) 조합 중 keydown이 중복 발생하는 문제는
...
숙련 필사자의 지면 관행을 관찰\footnote{국립국악원 발간 악보(취타·군악 등)를
기준 이미지로 삼았다. \TODO{기준으로 삼은 악보의 정확한 서지(간행물명·연도)
명시}}하여 다음 규칙들을 렌더링 엔진에 명문화했다.
...
\begin{itemize}
  ...
  \item \textbf{옥타브 변형 한자 우선}: 한자 모드에서는 潢(청황)·汏 등
...
쌓는 전역 단일 스택(⌘/Ctrl+Z)으로 통일하여, 장단·가사 초기화나 임시 저장
...
템포는 20--240\,BPM이다. 템포 표시를 켜면 첫 각 위에
...
\code{.jgb.json} 파일로 내보내고 불러온다. JSON 컨테이너 안에 3장의
