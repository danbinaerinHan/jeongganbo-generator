...
console.log("밑감 — 음이름과 조표를 옳게 푸는가");
{
  ...
  // 조표는 궁(宮)이 정한다 — 5음 음계는 임시표 없이 적히는 조표가 셋이라(♭5·♭4·♭3)
  // 잉크로는 못 고른다. 황=E♭ 평조(황태중임남)는 궁이 중려라 A♭장조(♭4).
  ...
  ok(`황=E♭ 계면조 → 내림표 6개(G♭장조, 궁=협종)`, f2 === -6, `나온 값: ${f2}`);
  ok("계면조도 임시표 없이",
     ["황", "협", "중", "임", "무"].every((n) => CORE.pitchAt(63 + YUL.indexOf(n), f2).acc === null));
  // 황=C 평조(C D F G A) — 궁은 중려(F)라 바장조(♭1). 기준음이 옮겨지면 조표도 함께 옮겨진다.
  ...
  // 궁을 못 짚는 음계는 **조표 없음**으로 물러난다 — 임시표가 가장 적은 조를 골라 주면
  // 못 짚은 자리에서 아무 조나 선언하는 셈이라, 읽는 사람이 그 조라고 믿게 된다.
  ok("반음이 붙어 궁을 못 짚으면 조표 없음", CORE.fifthsFor([0, 1, 2, 3, 4, 5, 6, 7, 8]) === 0,
