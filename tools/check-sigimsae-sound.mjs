...
console.log("\n조 판단 — 프리셋 없이도 곡의 조를 먼저 알아내 시김새가 그 음계로 움직이는가");
{
  // 2026-08-14 사용자 확정 — '한음 위'는 반음이 아니라 **그 곡 음계의 바로 윗음**이므로
  // 조 프리셋이 비어 있으면('12율 전체') 곡이 어느 조인지 먼저 판단해야 한다.
  // 같은 "황|{니}"라도 곡이 평조면 태, 계면조면 협이 나와야 한다.
  app.fields.joPreset = "all";
  eq("평조 곡으로 판단되면 {니}: 황 → 태", pitchesOf("황|{니}|태|중|임|남|황|임"),
     ["황", "태", "태", "중", "임", "남", "황", "임"].map(P));
  eq("계면조 곡으로 판단되면 {니}: 황 → 협", pitchesOf("황|{니}|협|중|임|무|황|임"),
     ["황", "협", "협", "중", "임", "무", "황", "임"].map(P));
  app.fields.joPreset = "hwang-pyeong";
}
