(function () {
  ...
  const TOUR_STEPS = [
    ...
    // 그린 악보**의 캡처다: 에디터에 "황 | 황 태 | 황태 | 황{미는표}"를 넣고 렌더된 페이지
    // SVG를 정간별로 viewBox 크롭 → canvas로 PNG 데이터 URL화(16px/mm, 흰 배경, 편집
    // 하이라이트 rect 제거). 렌더 모양이 바뀌면 같은 방법으로 다시 떠서 교체할 것.
  ...
  function tourGo(i, dir) {
    ...
    if (s.fig) {
      ...
      s.fig.forEach(function (ex) {
        ...
        item.appendChild(t); item.appendChild(a); item.appendChild(im); item.appendChild(c);
