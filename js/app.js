(function () {
  ...
  const ORN_LIST = [
    ...
    { s: "sigimsae-08", k: "s08", c: "wo" }, { s: "sigimsae-10", k: "s10", c: "wo" },
    { s: "sigimsae-11", k: "s11", c: "wo" },
    ...
    { s: "sigimsae-17", k: "s17", c: "wo" }, { s: "sigimsae-18", k: "s18", c: "wo" },
  ...
  const TOUR_STEPS = [
    ...
    // 본문은 리본의 실제 그룹 순서(입력 → 각 삽입/삭제 → 정간 편집 → 크기)를 그대로 따라간다
    { sel: "#melodyRibbon", title: "기능바",
      body: "• **입력** 그룹 — **율명·시김새·장단·가사/활** 팔레트를 엽니다. 정간에 **직접 타이핑**해도 되고, 팔레트에서 **골라 넣어도** 됩니다\n• **각 삽입/삭제** — 각(마디)을 원하는 자리에 넣거나 뺄 수 있습니다\n• **정간 편집** — 악보에서 드래그로 고른 구간의 내용을 지우거나, **정간 서식**으로 배경색 · 정간 합치기/나누기 · 가로줄 모양을 바꿀 수 있습니다\n• 율명·가사 **글자 크기**를 조절할 수 있습니다" },
    ...
    { sel: "#ornEditToggle", title: "시김새 편집(위치·삭제)", prep: tourEnsureOrnWin,
      body: "• 팔레트의 **⌖ 시김새 편집(위치·삭제)**을 켭니다\n• 악보에서 **시김새를 클릭**해 고르면(옅은 네모 표시) 위치·크기를 조절할 수 있습니다\n• **Backspace/Delete**로 고른 시김새를 바로 **지울** 수 있습니다 — 잘못 넣었을 때\n• 퇴성·추성처럼 **정간 끝쪽에 붙이려면** 여기서 좌우·상하를 옮깁니다\n• 글자로도 됩니다 — 괄호 안에 **@크기,좌우,상하**를 덧붙입니다 (예: **황{미는표@120,5,-5}**)" },
    ...
    // 켜는 자리(기능바 버튼)를 가리킨다.
    { sel: "#winToggleGakName", title: "빠르기 표기 · 각 이름 (章)",
      body: "• 기능바 **입력** 그룹의 **章 빠르기/각/장**에서 엽니다\n• 각(세로줄) 위에 **각·장 이름**(대여음·1장 등)을 붙일 수 있습니다\n• **빠르기 표기**(一分・N井)를 켜면 악보 머리에 빠르기가 **글자로 찍힙니다**\n• 이 빠르기는 **악보에 남는 표기**입니다 — 미리듣기 속도(⚙)와 달리 인쇄·PNG에 그려집니다" },
    ...
    { sel: "#cellStyleWin", title: "정간 서식 (배경색·가로줄)", prep: tourEnsureCellStyleWin,
      also: ["#winToggleCellStyle"],
      body: "• 기능바 **정간 편집** 그룹의 **정간 서식**에서 엽니다\n• 먼저 악보에서 **정간을 클릭·드래그로 고른 뒤** 버튼을 누릅니다\n• **배경색** — 색을 골라 고른 칸을 칠합니다\n• **정간** — 칸 **합치기** · 되돌리는 **나누기** · 통째로 **없애기**\n• **가로줄** — **위/아래**를 고르고 **굵게·점선·이중선**을 그 줄에 입힙니다\n• **초기화** — 그 칸에 해둔 배경색·가로줄을 모두 물립니다\n!! **두 줄짜리 마디선·덧줄**은 '가로줄 › 이중선'으로 그립니다 — 각 끝이든 정간 위아래든" },
    // 듣기 — 상단바 1급 버튼 셋(재생·정지·재생 설정)인데 예전 투어엔 통째로 빠져 있었다.
    // 악보 다음에 두는 건 '써 넣었으면 들어본다'는 차례라서(설정·인쇄보다 앞).
    { sel: "#playBar", title: "들어보기",
      body: "• **재생**을 누르면 써 넣은 선율을 소리로 들어볼 수 있습니다 (사인파 · 시김새 제외)\n• **재생 설정 ⚙**에서 기준음(황)과 **듣는 빠르기(BPM)**를 바꿀 수 있습니다\n• 이 빠르기는 **미리듣기 속도**만 바꿉니다 — 악보에 빠르기를 **적으려면** 위 **章** 창의 표기를 켜세요" },
    // '설정' 단계는 뺐다(2026-07-17) — '레이아웃 잡기'가 이미 사이드바를 통째로 비춰
    // 겹쳤고, 문서 탭(제목·종이 방향)은 따로 가르칠 만큼 헷갈리지 않다. 보관 탭의
    // 임시 저장만 아래 '인쇄 · 파일' 단계에 한 줄로 흡수.
    { sel: "#outBox", title: "새 문서 · 인쇄 · 파일",
      body: "• **새 문서** — 새 곡을 시작합니다. 지금 작업은 임시저장할지 먼저 묻습니다\n• **인쇄** — 완성한 악보를 종이나 **PDF**로 출력할 수 있습니다\n• **⋯ 파일** 메뉴 — **PNG** 다운로드 · 파일 저장·불러오기를 할 수 있습니다\n• 여러 버전을 남기고 싶으면 사이드바 **보관** 탭에 이름 붙여 저장할 수 있습니다\n!! **작업은 자동 저장됩니다** — 실수로 새로고침하거나 창을 닫아도 그대로 남습니다" },
  ...
  // '정간 서식' 단계용 — 시김새 창과 같은 규칙(tourPrevWin 공유)으로 셀 서식 창을 열어
  // 배경색·정간·가로줄·초기화 구획을 열린 채 보여준다. endTour가 원래 창으로 복원.
  function tourEnsureCellStyleWin() {
    const w = $("cellStyleWin");
    if (!w || w.classList.contains("win-open")) return;
    if (!tourTouchedWin) {
      const open = document.querySelector(".direct-win.win-open");
