(function () {
  ...
  function setCursorMode(mode) {
    ...
    // 모드에서는 그 파란 상자로 할 수 있는 일이 하나도 없어, 남아 있으면 '아직 골라져
    // 있다'는 거짓말만 한다 — 노란 '편집 자리' 표시를 입력 모드에서만 그리는 것과 같은
    // 잣대다. 여기서만 **비우고**(그리기만 접는 게 아니라) 다시 그린다.
    //  · 어느 모드에서 골랐든 마찬가지다 — 같은 파란 상자가 온 데를 거쳐 왔다고 다르게
    //    굴면 그 표시가 무슨 뜻인지 흐려진다.
    //  · 클립보드(melClip·lyClip)는 그대로 둔다 — 복사해 둔 것은 선택과 다른 물건이라
    //    이동으로 화면을 옮긴 뒤 자리를 다시 골라 붙여넣을 수 있어야 한다.
    if (mode === "pan" && hasSel()) { clearSel(); render(); return; }
  ...
  // 곁줄에서 자주 쓰는 활 표시(가로표·세로표)를 **글자로 바로** 적을 수 있게 한다
  // (2026-08-26 사용자 요청): `--` → {가로표}, `|` → {세로표}.
  // 여섯 자짜리 `{가로표}`를 매번 치는 게 번거로워 둔 지름길이고, 담기는 것은 예전과 같은
  // 토큰이라 팔레트로 넣은 것과 한 글자도 다르지 않다(옛 악보도 그대로 그려진다).
  // ★ **`|`는 곁줄 원문에서 정간을 가르는 글자**다. 아래 줄이 그걸 공백으로 걷어내고 있어
  //   예전에는 `|`를 쳐도 조용히 사라졌는데, 그 **앞에서** 토큰으로 바꾸므로 이제 안 사라진다.
  //   순서를 뒤바꾸지 말 것.
  // 단일 `-`는 그대로 둔다 — 정간 줄에서 '이음'이라 뜻이 이미 있고, 곁줄에서도 그 글자를
  //   그대로 쓰는 사람이 있다. 셋 이상(`---`)은 하나로 본다.
  function lyricShorthand(v) {
    return String(v).replace(/-{2,}/g, "{가로표}").replace(/\|/g, "{세로표}");
  }
  function setLyricText(gi, ci, val) {
    val = lyricShorthand(val).replace(/[|\n]+/g, " ").replace(/\s+/g, " ").trim();
  ...
  const OCT_HANJA = {
    ...
    // ★ 유(-1)는 일부러 비워 둔다. docs/Cha-Unicode.docx의 Oct -1 칸에 유와 이가 **둘 다**
    //   侇(U+4F87)로 적혀 있는데, 侇는 亻+夷라 **이(夷)의 글자**다(유는 亻+蕤라야 한다).
    //   문서의 오기로 보이고 亻+蕤에 해당하는 유니코드 글자는 확인하지 못했다. 그대로 두면
    //   배유와 배이가 같은 글자로 그려지므로, 없는 셈 치고 아래 폴백(기본자 + 옥타브 점,
    //   drawGlyph의 noVariant)에 맡긴다 — 하배이·중청대가 이미 그렇게 그려진다.
    "-1": { 황: "僙", 대: "㐲", 태: "㑀", 협: "俠", 고: "㑬", 중: "㑖", 임: "㑣", 이: "侇", 남: "㑲", 무: "㒇", 응: "㒣" },
  ...
  const SYM_LANES = {
    ...
    lyric: {
      ...
      syntax: { how: "괄호 안에 넣어 표시 · 활은 -- 가로표 · | 세로표", ex: ["(덩)", "{덩}", "[덩]"] },
