...
(function (root) {
  ...
  // 조표 고르기 — **궁(宮)이 어디인가**로 정한다. 잉크가 가장 적은 조표가 아니다.
  //
  // 5음 음계는 임시표 없이 적을 수 있는 조표가 하나가 아니라 **셋**이다(황종 평조면
  // ♭5·♭4·♭3이 다 된다). 셋 다 같은 다섯 음을 적으므로 음정은 어느 쪽이든 안 틀리고,
  // 갈리는 것은 **무엇을 do로 선언하는가**뿐이다. 그러니 이건 표기 경제의 문제가 아니라
  // 악조 해석의 문제다 — 관행은 궁을 으뜸음으로 삼는 조표를 쓴다.
  //   황종 평조(황태중임남) → 궁은 중려라 ♭4,  중려 평조(중임무황태) → 궁은 무역이라 ♭5,
  //   황종 계면조(황협중임무) → 라·도·레·미·솔이라 협종이 do → ♭6.
  // 예전엔 '임시표가 가장 적은 것'을 골라 셋 다 한 겹씩 모자랐다(각각 ♭3·♭4·♭5).
  // 음이 하나도 안 틀려 검사로는 안 잡히고 악보를 읽는 사람 눈에만 보이던 오류다.
  //
  // 궁은 **5도권에서 가장 낮은 음계음**이다 — 곧 그 음은 음계에 있는데 한 겹 더 내림쪽
  // 음은 음계에 없는 자리. 평조·계면조가 이 규칙 하나로 함께 맞는다.
  ...
  function fifthsFor(pcs) {
    ...
    for (let f = -7; f <= 7; f++) {
      ...
      if (has[fifthPc(f)] && !has[fifthPc(f - 1)]) fits.push(f);   // 궁 = 5도권 맨 아래 음계음
    }
